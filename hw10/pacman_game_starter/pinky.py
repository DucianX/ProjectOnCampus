...
class Pinky(GameCharacter):
    ...
    def update(self):
        ...
        # if the pacman is on the upperleft
        if (up_down_part <= 0 and left_right_part <= 0 and abs(up_down_part) > 5):
            if (on_horz and on_vert):
                self.x_add = 0
                self.y_add = -self.velocity
        if (up_down_part <= 0 and left_right_part <= 0 and abs(left_right_part) > 5):
            if (on_horz and on_vert):
                self.x_add = -self.velocity
                self.y_add = 0
        
        # if the pacman is on the lowerleft
        if (up_down_part >= 0 and left_right_part <= 0 and abs(up_down_part) > 5):
            if (on_horz and on_vert):
                self.x_add = 0
                self.y_add = self.velocity
        ...
        # if the pacman is on the lowerright
        if (up_down_part >= 0 and left_right_part >= 0 and abs(left_right_part) > 5):
            if (on_horz and on_vert):
                self.y_add = 0
                self.x_add = self.velocity
